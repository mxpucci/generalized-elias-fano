//
// Fast decoder for sequences where runs of zeros are terminated by a 1-bit.
//

#ifndef GEF_FAST_ZERO_TERMINATED_DECODER_HPP
#define GEF_FAST_ZERO_TERMINATED_DECODER_HPP
...
namespace gef {
    class FastZeroTerminatedDecoder {
    private:
    ...
    public:
        FastZeroTerminatedDecoder(const uint64_t* data, size_t total_bits, size_t start_pos = 0)
            : data_(data),
        ...
        inline uint64_t next() {
            ...
            while (pos_ < total_bits_) {
                ...
                if (chunk != 0ULL) {
                    const uint32_t one_offset = static_cast<uint32_t>(__builtin_ctzll(chunk));
                    pos_ += one_offset + 1;
                    count += one_offset;
                    return count;
                }
        ...
        size_t next_batch(uint32_t* out, size_t max_batch) {
            ...
            while ((produced + lane_fill) < max_batch && pos_ < total_bits_) {
                ...
                uint64_t chunk = (data_[word_idx] >> bit_offset) & mask_for_bits(bits_remaining_in_word);
                ...
                while (bits_remaining_in_word > 0) {
                    ...
                    const uint32_t one_offset = static_cast<uint32_t>(__builtin_ctzll(chunk));
                    pending_gap += one_offset;
                    pos_ += one_offset + 1;
                    bits_remaining_in_word -= one_offset + 1;
                    chunk >>= (one_offset + 1);
                    chunk &= mask_for_bits(bits_remaining_in_word);
            ...
            if (pos_ >= total_bits_ && pending_gap > 0 && (produced + lane_fill) < max_batch) {
                emit_gap(static_cast<uint32_t>(pending_gap));
                pending_gap = 0;
            }
